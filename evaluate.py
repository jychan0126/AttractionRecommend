...
def evaluate_model(model, testRatings, testNegatives, K, num_thread):
    ...
    attr = [""] * 3487    # Need to fix
    for a, i in attr2index.iteritems():
        attr[i] = a


    for idx in range(len(_testRatings)):
        (hr,ndcg) = eval_one_rating(idx, typeAttraction, attr, type2index)
        hits.append(hr)
        ndcgs.append(ndcg)      
    return (hits, ndcgs)

def eval_one_rating(idx, typeAttraction, attr, type2index):
    ...
    for i in range(len(items)):
        for typ, a in typeAttraction.iteritems():
            if attr[items[i]] in a:
                types.append(type2index.index(typ))
                break

    predictions = _model.predict([users, np.array(items), np.array(types)],batch_size=100, verbose=0)
    for i in range(len(items)):
        item = items[i]
